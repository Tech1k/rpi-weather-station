...
class WeatherStation(CarouselContainer):
    ...
    READINGS_PRINT_TEMPLATE = 'Temp: %sF, Humidity: %s%%, Pressure: %s inHg'
    ...
    def get_temperature(self):
        ...
        f_avg_temp = round(self.to_fahrenheit(avg_temp), 1)
        
        # Get the CPU temperature
        cpu_temp = self._get_cpu_temp()

        f_cpu_temp = round(self.to_fahrenheit(cpu_temp), 1)
        ...
        f_adj_temp = round(self.to_fahrenheit(adj_temp), 1)

        print('\033[92mCPU temp: %s, Avg temp: %s, Adj temp: %s\033[0m' % (f_cpu_temp, f_avg_temp, f_adj_temp))
